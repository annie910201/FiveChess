...
public final class AIDect {
	...
	public final void setLevel(int[][] computer,int[][] player) {
		...
		for(int i=0;i<SIZE;i++) {
			for(int j=0;j<SIZE;j++) {
				...
				if((j+sum>10||i+sum>10)&&i-1>=0&&j-1>=0) {
					switch(sum) {
					case 1:
				...
				else if((j-1<0||i-1<0)&&i+sum<=10&&j+sum<=10) {
					switch(sum) {
					case 1:
				...
				else if(i+sum<=10&&j+sum<=10&&i-1>=0&&j-1>=0) {
					switch(sum) {
					case 1:
				...
				if((i+sum>10||j-sum<0)&&i-1>=0&&j+1<=10) {
					switch(sum) {
					case 1:
						if(computer[i-1][j+1]==0&&level[i][j]<levelList[1]) {
							level[i][j]=levelList[1];
				...
				else if((j+1>10||i-1<0)&&i+sum<=10&&j-sum>=0) {
					switch(sum) {
					case 1:
						if(computer[i+sum][j-sum]==0&&level[i][j]<levelList[1]) {
							level[i][j]=levelList[1];
				...
				else if(i+sum<=10&&j-sum>=0&&i-1>=0&&j+1<=10) {
					switch(sum) {
					case 1:
						if(computer[i+sum][j-sum]==0&&computer[i-1][j+1]==0&&level[i][j]<levelList[2]) {
							level[i][j]=levelList[2];
	...
	public final void checkBothSide(int[][] computer,int [][]player) {
		...
		//判斷兩邊放置情況
		//rank=1一邊有，rank=2兩邊都沒有
		for(int i=0;i<SIZE;i++) {
			for(int j=0;j<SIZE;j++) {
				if(level[i][j]!=maxLevel)continue;
					//判斷直的
					if(direction[i][j]==82) {
						if(i+conMax>10&&i-1>=0) {
							if(player[i-1][j]==0&&computer[i-1][j]==0)rank[i-1][j]=1;
						}
						else if(i+conMax<=10&&i-1<0) {
							if(player[i+conMax][j]==0&&computer[i+conMax][j]==0)rank[i+conMax][j]=1;
						}
					...
					//判斷橫的
						...
						if(j+conMax>10&&j-1>=0) {
							if(player[i][j-1]==0&&computer[i][j-1]==0)rank[i][j-1]=1;
						}
						else if(j+conMax<=10&&j-1<0) {
							if(player[i][j+conMax]==0&&computer[i][j+conMax]==0)rank[i][j+conMax]=1;
						}
					...
					//判斷左上右下
						...
						if((j+conMax>10||i+conMax>10)&&i-1>=0&&j-1>=0) {
							if(player[i-1][j-1]==0&&computer[i-1][j-1]==0)rank[i-1][j-1]=1;
						}
						else if((i-1<0||j-1<0)&&i+conMax<=10&&j+conMax<=10) {
							if(player[i+conMax][j+conMax]==0&&computer[i+conMax][j+conMax]==0)rank[i+conMax][j+conMax]=1;
						}
					...
					//判斷右上左下
						...
						if((j-conMax<0||i+conMax>10)&&i-1>=0&&j+1<=10){
							if(player[i-1][j+1]==0&&computer[i-1][j+1]==0)rank[i-1][j+1]=1;
						}
						else if((i-1<0||j+1>10)&&i+conMax<=10&&j-conMax>=0) {
							if(player[i+conMax][j-conMax]==0&&computer[i+conMax][j-conMax]==0)rank[i+conMax][j-conMax]=1;
						}
